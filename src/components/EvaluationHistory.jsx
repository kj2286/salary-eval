    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-5">
      ...
      <ul className="mt-3 flex items-end gap-1.5">
        {quarters.map((q) => {
          ...
          const grade = record ? (GRADE_MAP[record.grade] ?? GRADE_MAP.B) : null
          const height = record ? 12 + ((Number(record.score) - 1) / 4) * 52 : 6
          const isCurrent = q === quarter
          return (
            <li key={q} className="flex flex-1 flex-col items-center gap-1">
              <span className="text-[10px] font-medium tabular-nums text-slate-500">
                {record ? Number(record.score).toFixed(1) : '—'}
              </span>
              <button
                type="button"
                onClick={() => onSelectQuarter(q)}
                title={`${quarterLabel(q)}${record ? ` · ${grade.key}등급 · ${Number(record.score).toFixed(2)}점` : ' · 미평가'}`}
      ...
      {history.length ? (
        <p className="mt-3 text-[11px] text-slate-400">
          누적 {history.length}개 분기 · 연간 보상은 이 기록들의 평균으로 확정합니다.
        </p>
      ) : (
